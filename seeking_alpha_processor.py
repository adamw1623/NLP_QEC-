...
class session:
    ...
    def getExecs(self, org_df):
        print("Getting Execs...")
        Recording = False
        Asterik_Format = False
        for line in self.string_list:
            if not Recording:
                if 'Executives\n' in line or 'Corporate Participants\n' in line or 'CORPORATE PARTICIPANTS\n' in line:
                    Recording = True
            else:
                if line[0] == '*':
                    Asterik_Format = True
                if line == 'Analysts\n' or line == 'Conference Call Participants\n' or 'CONFERENCE CALL PARTICIPANTS\n' == line:
                    break
                else:
                    try:
                        if not Asterik_Format:
                            ...
                            if "CEO" in exec_title or "Chief Executive Officer" in exec_title or "Chief Executive officer" in exec_title:
                                self.CEOname = exec_name
                        if '*' in line and Asterik_Format:
                            self.Execs.append(line[2:-1])
                    except:
                        try:
                            exec_name = line.split('–')[0]
                            exec_title = line.split('–')[1]
        ...
        if self.CEOname == "empty":
            company_df = org_df[org_df['Ticker'] == self.Ticker]
            ceo_name = company_df["CEO_Name"].values[1]
            for exec_ in self.Execs:
                isceo = True
                for name in ceo_name.split():
                    if name not in exec_:
    ...
    def IsolateInterview(self):
        ...
        for string in self.string_list:
            ...
            and "Session\n" in string) \
            or ("Questions" in string and "and" in string and "Answers" in string):
...
def CreateDataframe(folder_path, org_df):
    ...
    for file in folder:
        ...
        date = file.Date
        print(Ticker)
        wiki_row = wiki_table[wiki_table[1] == Ticker]
        try:
